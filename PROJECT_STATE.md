...
> Snapshot updated **2026-08-24**: **M5 code-complete & locally verified** (lint/typecheck/212 unit tests/build
> green; E2E Chromium-only). Original audit 2026-08-23 by static inspection only; the live-validation addendum
> of that day and this M5 update supersede earlier claims.
...
- **Current phase:** Beta-track MVP development. **M1–M5 code-complete per `tasks.md`** (ticked 2026-08-24);
  migrations 0001–0005 applied & live-verified 2026-08-23 (RLS enforced); migrations 0006–0009 committed &
  pending owner apply via Supabase dashboard. Locally verified 2026-08-24: lint/typecheck/212 unit tests/build
  green; E2E on Chromium only. Remaining open: Vercel deployment + live apply of 0006–0009. Next up: M6.
...
**Rough completion estimate:** ~50% overall.
Basis: 9 milestones defined in tasks.md (M1–M9). Per tasks.md checkboxes M1–M5 done ≈ 55%; weighting the
remaining hardening/beta/Phase-2 work (and M5's pending live migration apply), ~50% is a fair midpoint.
...
3. Apply supabase/migrations/0001-0005 — DONE & live-verified 2026-08-23. **Immediate open action: apply
   0006–0009 to the live project (owner dashboard; no CLI installed), then verify RLS.** Enable Google OAuth +
   phone OTP providers in the Supabase console.
...
(paid + debt == total), balance aggregation~~ — DONE 2026-08-24 (M5 code-complete; migrations 0006–0009 pending live apply, see item 3). **Next up: M6 obligations** (item 9).
...
(helpers already exist in src/utils/subscription.ts). 10. ~~Inventory feature (product list shelf/warehouse + manual move dialog) replacing InventoryView stub~~ — DONE 2026-08-24 as part of M5.
